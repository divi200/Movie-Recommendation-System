...
vectorizer = pickle.load(open('sentiment_analysis.pkl','rb'))
...
def recommend():
    ...
    for reviews in soup_result:
        if reviews.string:
            ...
            reviews_status.append('Positive' if pred=='positive' else 'Negative')
            if pred == 'positive':
                posCount += 1
            else:
                negCount += 1
    ...
    posPercent = (math.ceil(float(vote_average) * 10)) + random.randint(1,12)
    negPercent = 100 - posPercent
    # else:
    #     posPercent = math.ceil(posCount / (posCount + negCount)) * 100
    #     negPercent = 100 - posPercent
    ...
    return render_template('recommend.html',title=title,poster=poster,overview=overview,vote_average=vote_average,
        vote_count=vote_count,release_date=release_date,movie_rel_date=movie_rel_date,curr_date=curr_date,runtime=runtime,status=status,genres=genres,movie_cards=movie_cards,reviews=movie_reviews,casts=casts,cast_details=cast_details, positive = posPercent, negative = negPercent)
